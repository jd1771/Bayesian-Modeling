...
model <- ulam(
  alist(
    effort ~ dnorm(mu, sigma),
    mu <- a + b_complexity * cmplx + b_year * year + 
      b_category * cat2 + b_analyst * acap + 
      b_programmer * pcap,
    a ~ dnorm(0, 10),
    b_complexity ~ dnorm(0, 1),
    b_year ~ dnorm(0, 1),
    b_category ~ dnorm(0, 1),
    b_analyst ~ dnorm(0, 1),
    b_programmer ~ dnorm(0, 1),
    sigma ~ dexp(1)
  ),
  data = new_df
)
